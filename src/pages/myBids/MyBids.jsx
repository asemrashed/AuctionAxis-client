...
const MyBids = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      //     console.log(err);
      //   });
      axiosSecure.get(`/bids/?user_email=${email}`)
      .then(res => {
        setBids(res.data)
      })
    }
  ...
  return (
    <div className="max-w-[1440px] min-h-[50vh] mx-auto bg-white py-5 md:py-10 px-3 md:px-5">
      <h1 className="text-2xl md:text-4xl font-bold text-center mb-3 md:mb-5 text-gray-800">My Bids :<span className="text-primary"> {bids.length}</span></h1>
      <div>
        {/* Header */}
